...
class TestUpstreamCommand extends TerminusCommand implements SiteAwareInterface
{
  ...
  /**
   * Pushed upstream updates from a specific branch to a multi-dev
   *
   * @authorize
   *
  ...
  public function testUpdate($site_id, $options = [
    ...
    $this->passthru("git --git-dir='{$git_location}/.git' checkout -b {$env}");
    $this->log()->notice('Merging in master environment');
    $this->passthru("git --git-dir='{$git_location}/.git' merge master");
    $this->log()->notice('Pulling repo {repo} and branch {branch}', ['repo' => $repo, 'branch' => $branch]);
    ...
    $this->passthru("git --git-dir='{$git_location}/.git' push origin {$env}");
